...
ModelerConsumers::ModelerConsumers(std::shared_ptr<Modeler> modeler) : Entity()
{
	...
	xConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->X(true); });
	yConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->Y(true); });
	zConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->Z(true); });
	tConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->T(true); });
	lConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->L(); });
	xCapConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->X(false); });
	yCapConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->Y(false); });
	zCapConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->Z(false); });
	tCapConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->T(false); });
	...
	runConsumer_ = std::make_shared<EventConsumer<void(void)>>([this, modeler]() { modeler->Run(); });
}
